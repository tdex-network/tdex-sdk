...
var proto = require('tdex-protobuf/js/swap_pb');
var JSBI = _interopDefault(require('jsbi'));
var axios = _interopDefault(require('axios'));
var grpc = require('@grpc/grpc-js');
var services = require('tdex-protobuf/js/trade_grpc_pb');
var messages = require('tdex-protobuf/js/trade_pb');
...
var Swap = /*#__PURE__*/function (_Core) {
  ...
  _proto.accept = function accept(_ref2) {
    ...
    var msgRequest = proto.SwapRequest.deserializeBinary(message); // Build Swap Accepr message
...
var WatchOnlyWallet = /*#__PURE__*/function () {
  function WatchOnlyWallet(_ref) {
    ...
    var payment = liquidjsLib.payments.p2wpkh({
      address: address,
      network: network
    });
  ...
  _proto.updateTx = function updateTx(psbtBase64, inputs, inputAmount, outputAmount, inputAsset, outputAsset) {
    ...
    unspents.forEach(function (i) {
      return psbt.addInput({
        ...
        witnessUtxo: {
          ...
          value: liquidjsLib.confidential.satoshiToConfidentialValue(i.value),
    ...
    psbt.addOutput({
      ...
      value: liquidjsLib.confidential.satoshiToConfidentialValue(outputAmount),
    ...
    if (change > 0) {
      psbt.addOutput({
        script: Buffer.from(this.script, 'hex'),
        value: liquidjsLib.confidential.satoshiToConfidentialValue(change),
        asset: Buffer.concat([Buffer.from('01', 'hex'), Buffer.from(inputAsset, 'hex').reverse()]),
        nonce: Buffer.from('00', 'hex')
      });
    }
...
var Wallet = /*#__PURE__*/function (_WatchOnlyWallet) {
  ...
  function Wallet(_ref2) {
    ...
    if (!keyPair) _this2.keyPair = liquidjsLib.ECPair.makeRandom({
...
function fromAddress(address, network) {
  var _network = network ? liquidjsLib.networks[network] : liquidjsLib.networks.liquid;
...
function fromRandom(network) {
  ...
  try {
    var keyPair = liquidjsLib.ECPair.makeRandom({
      network: _network
    });

    var _payments$p2wpkh = liquidjsLib.payments.p2wpkh({
...
function fromWIF(wif, network) {
  ...
  try {
    var keyPair = liquidjsLib.ECPair.fromWIF(wif, _network);

    var _payments$p2wpkh2 = liquidjsLib.payments.p2wpkh({
      pubkey: keyPair.publicKey,
      network: _network
    }),
...
var TraderClient = /*#__PURE__*/function () {
  function TraderClient(providerUrl) {
    ...
    this.client = new services.TradeClient(providerUrl, grpc.credentials.createInsecure());
  }
  /**
   * tradePropose
   * @param market
  ...
  _proto.tradePropose = function tradePropose(_ref, tradeType, swapRequestSerialized) {
    ...
    return new Promise(function (resolve, reject) {
      var market = new messages.Market();
  ...
  _proto.markets = function markets() {
    ...
    return new Promise(function (resolve, reject) {
      _this3.client.markets(new messages.MarketsRequest(), function (err, response) {
  ...
  _proto.buy = function buy(_ref) {
    ...
    try {
      ...
      if (!privateKey) {
        ...
        return Promise.resolve(_this3.marketOrderRequest(market, exports.TradeType.BUY, amount, watchOnlyWallet));
      } else {
        var wallet = Wallet.fromWIF(privateKey, _this3.chain);
        return Promise.resolve(_this3.marketOrderRequest(market, exports.TradeType.BUY, amount, wallet)).then(function (swapAccept) {
  ...
  _proto.sell = function sell(_ref2) {
    ...
    try {
      ...
      if (!privateKey) {
        ...
        return Promise.resolve(_this5.marketOrderRequest(market, exports.TradeType.SELL, amount, watchOnlyWallet));
      } else {
        var wallet = Wallet.fromWIF(privateKey, _this5.chain);
        return Promise.resolve(_this5.marketOrderRequest(market, exports.TradeType.SELL, amount, wallet)).then(function (swapAccept) {
  ...
  _proto.preview = function preview(market, tradeType, amountInSatoshis) {
    try {
      ...
      return Promise.resolve(_this7.grpcClient.balances({
        ...
        if (tradeType === exports.TradeType.BUY) {
...
exports.WatchOnlyWallet = WatchOnlyWallet;
exports.calculateExpectedAmount = calculateExpectedAmount;
exports.calculateProposeAmount = calculateProposeAmount;
