import { isConfidentialOutput, unblindOutput, greedyCoinSelector, address, isValidAmount } from 'ldk';
export * from 'ldk';
import { Psbt, Transaction, confidential } from 'liquidjs-lib';
import * as proto from 'tdex-protobuf/generated/js/swap_pb';
import { SwapRequest, SwapAccept, SwapComplete } from 'tdex-protobuf/generated/js/swap_pb';
import { credentials } from '@grpc/grpc-js';
import { TradeClient } from 'tdex-protobuf/generated/js/trade_grpc_pb';
import { TradeProposeRequest, TradeCompleteRequest, MarketsRequest, MarketPriceRequest, BalancesRequest } from 'tdex-protobuf/generated/js/trade_pb';
import { Market } from 'tdex-protobuf/generated/js/types_pb';
  ...
  try {
    ...
    return Promise.resolve(Promise.all(filteredByWitness.map(function (i) {
      try {
        ...
        var _temp6 = function () {
          if (i.witnessUtxo && isConfidentialOutput(i.witnessUtxo)) {
            ...
            return Promise.resolve(unblindOutput({
      ...
      var queryValues = filteredByAsset.map(function (i) {
        var valAsNumber = i.witnessUtxo.value instanceof Buffer ? confidential.confidentialValueToSatoshi(i.witnessUtxo.value) : i.witnessUtxo.value;
        return valAsNumber;
    ...
    return Promise.resolve(outputs.some(function (o) {
      try {
        ...
        var _temp3 = function _temp3(_result2) {
          ...
          var isValue = confidential.confidentialValueToSatoshi(o.value) === value;
          return isAsset && isValue;
        };

        // unblind first if confidential ouput
        var isConfidential = isConfidentialOutput(o);
  ...
  try {
    ...
    decodedFromRequest.psbt.data.inputs.forEach(function (i, inputIndex) {
      if (!i.witnessUtxo && i.nonWitnessUtxo) {
        ...
        var witnessUtxo = Transaction.fromHex(i.nonWitnessUtxo).outs[vout];
...
var Swap = /*#__PURE__*/function (_Core) {
  ...
  _proto.accept = function accept(_ref4) {
    ...
    try {
      ...
      var msgRequest = SwapRequest.deserializeBinary(message); // Build Swap Accept message
...
var SwapTransaction = /*#__PURE__*/function () {
  ...
  _proto.create = function create(unspents, amountToBeSent, amountToReceive, assetToBeSent, assetToReceive, addressForSwapOutput, addressForChangeOutput, coinSelector) {
    ...
    var receivingScript = address.toOutputScript(addressForSwapOutput, this.network).toString('hex'); // The receiving output
    ...
    if (changeOutputs.length > 0) {
      changeOutputs.forEach(function (changeOutput) {
        var changeScript = address.toOutputScript(changeOutput.address, _this.network).toString('hex'); // Change
...
var TradeCore = /*#__PURE__*/function (_Core) {
  ...
  _proto.buy = function buy(_ref) {
    ...
    try {
      ...
      return Promise.resolve(_this3.marketOrderRequest(market, TradeType.BUY, amount, asset, identity)).then(function (swapAccept) {
  ...
  _proto.sell = function sell(_ref2) {
    ...
    try {
      ...
      return Promise.resolve(_this5.marketOrderRequest(market, TradeType.SELL, amount, asset, identity)).then(function (swapAccept) {
...
var TraderClient = /*#__PURE__*/function () {
  function TraderClient(providerUrlString) {
    var creds = credentials.createInsecure();
    this.providerUrl = providerUrlString;
    var url = new URL(providerUrlString);
    this.client = new TradeClient(this.providerUrl, creds);

    if (url.protocol.includes('https')) {
      creds = credentials.createSsl();
      this.client = new TradeClient(url.host, creds);
    }
  }
  /**
   * tradePropose
  ...
  _proto.marketPrice = function marketPrice(_ref2, tradeType, amount, asset) {
    ...
    market.setBaseAsset(baseAsset);
    market.setQuoteAsset(quoteAsset);
    var request = new MarketPriceRequest();
    request.setMarket(market);
    request.setType(tradeType);
    request.setAmount(amount);
    request.setAsset(asset);
    return new Promise(function (resolve, reject) {
      _this4.client.marketPrice(request, function (err, response) {
        if (err) return reject(err);
        var list = response.getPricesList().map(function (priceWithFee) {
          return priceWithFee.toObject();
        });
        resolve(list);
      });
    });
  ...
  _proto.balances = function balances(_ref3) {
    ...
    request.setMarket(market);
    return new Promise(function (resolve, reject) {
      _this5.client.balances(request, function (err, response) {
        if (err) return reject(err);
...
export { Swap, SwapTransaction, Trade, TradeCore, TradeType, TraderClient, blindKeysMap, throwErrorIfSwapFail };
