...
export class SwapTransaction implements SwapTransactionInterface {
  ...
  async create(
    ...
    const utxos = onlyWithUnblindingData.filter(
      utxo => utxo.unblindData?.asset.toString('hex') === assetToBeSent
    );
    ...
    const receivingScript = address.toOutputScript(
    ...
    if (address.isConfidential(addressForSwapOutput)) {
      ...
      updaterOutReceive.blindingPublicKey = address.fromConfidential(
        addressForSwapOutput
      ).blindingKey;
    ...
    if (changeOutputs.length > 0) {
      const changeScript = address.toOutputScript(
      ...
      for (const { asset, amount } of changeOutputs) {
        ...
        if (address.isConfidential(addressForSwapOutput)) {
          ...
          updaterOutChange.blindingPublicKey = address.fromConfidential(
            addressForSwapOutput
          ).blindingKey;
  ...
  private _deriveBlindingKey(
    ...
    if (!derived.publicKey || !derived.privateKey)
      throw new Error('Could not derive blinding key');
