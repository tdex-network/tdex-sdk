...
export class TraderClient implements TraderClientInterface {
  ...
  constructor(
    ...
    this.client = new services.TradeServiceClient(new GrpcWebFetchTransport({
  ...
  async proposeTrade(
    ...
    if (call.response.swapFail) {
      throw new Error(`SwapFail for message id=${call.response.swapFail.id}. Failure code ${call.response.swapFail.failureCode} | reason: ${call.response.swapFail.failureMessage}`)
    }
  ...
  async markets(): Promise<Array<{ baseAsset: string; quoteAsset: string; feeBasisPoint: number }>> {
    ...
    return call.response!.markets.map(
  ...
  async marketPrice(
    ...
    return call.response!.previews.map(
